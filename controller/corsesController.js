...
module.exports.addCourse = asynchandler(async (req, res) => {
    ...
    try {
        ...
        if (!req.files || !req.files.image || !req.files.videos || req.files.videos.length === 0) {
        ...
        let videoTitles = [];
        try {
            videoTitles = req.body.videoTitles ? JSON.parse(req.body.videoTitles) : [];
        } catch (err) {
            console.error("Error parsing videoTitles:", err);
            return res.status(400).json({ error: 'تنسيق عناوين الفيديوهات غير صحيح' });
        }
